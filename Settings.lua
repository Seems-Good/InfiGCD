...
panel:SetScript("OnShow", function(self)
    ...
    -- Section: Behaviour ---------------------------------------------------
    local secBehaviour = Header(self, "Behaviour", y)
    y = y - LINE_H + 4

    local instantCB = CreateFrame("CheckButton", nil, self, "InterfaceOptionsCheckButtonTemplate")
    instantCB:SetPoint("TOPLEFT", self, "TOPLEFT", MARGIN + INDENT, y)
    instantCB.Text:SetText("Show on Instant Casts Only")
    instantCB:SetChecked(InfiGCDDB and InfiGCDDB.instantOnly or false)
    instantCB:SetScript("OnClick", function(btn)
        ns.SetInstantOnly(btn:GetChecked())
    end)
    self._instantCB = instantCB
    y = y - LINE_H

    local instantNote = SubText(self,
        "When enabled, cast-time spells (e.g. Steady Shot) are skipped.\n" ..
        "Only abilities that fire instantly will trigger the GCD display.", y)
    y = y - LINE_H - 10
...
panel:SetScript("OnShow", (function(original)
    return function(self)
        ...
        if self._instantCB and InfiGCDDB then
            self._instantCB:SetChecked(InfiGCDDB.instantOnly or false)
        end
    end
